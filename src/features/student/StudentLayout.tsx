...
import { STUDENT_NAV, STUDENT_NAV_GROUPS } from './navigation';
import { useStudentUnreadNotifications } from './hooks/useStudentQueries';
...
function SidebarContent({ onNavigate }: { onNavigate?: () => void }) {
  return (
    <div className="flex h-full flex-col">
      <Link to="/student" className="flex items-center gap-2.5 px-5 py-5" onClick={onNavigate}>
        ...
        {STUDENT_NAV_GROUPS.map((group) => (
          <div key={group} className="mb-4">
            ...
            <div className="space-y-0.5">
              {STUDENT_NAV.filter((n) => n.group === group).map((item) => (
                <NavLink
                  key={item.to}
                  to={item.to}
                  end={item.to === '/student'}
